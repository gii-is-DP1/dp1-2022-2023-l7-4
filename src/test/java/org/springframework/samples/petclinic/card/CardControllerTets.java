...
@WebMvcTest(controllers = CardController.class, excludeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE, classes = WebSecurityConfigurer.class), excludeAutoConfiguration = SecurityConfiguration.class)
public class CardControllerTets {
	...
	public void testShowCard() throws Exception {
		mockMvc.perform(get("/{cardId}", TEST_CARD_ID)).andExpect(status().isOk())
